...
class QuickNES final : public Emulator
{
public:
#ifdef __JAFFAR_USE_QUICKNES_ARKANOID
  static std::string getName() { return "QuickNESArkanoid"; }
#else
  static std::string getName() { return "QuickNES"; }
#endif
  ...
  jaffar::InputParser* getInputParser() const override { return _quickNES->getInputParser(); }

  void initializeImpl() override
  {
    // Setting game's internal video buffer
    ((emulator_t*)_quickNES->getInternalEmulatorPointer())->set_pixels(_videoBuffer, DEFAULT_WIDTH + 8);
    ...
    _quickNES->loadROM((uint8_t*)romFileData.data(), romFileData.size());
    ...
    _quickNES->setNTABBlockSize(_NTABBlockSize);

    // Getting input parser from the internal emulator
    const auto inputParser = _quickNES->getInputParser();
    ...
    if (_initialRAMDataFilePath != "")
    {
      ...
      memcpy(_quickNES->getLowMem(), initialRAMDataString.data(), 0x800);
    }
  }
  ...
  {
    if (propertyName == "LRAM") return property_t(_quickNES->getLowMem(), _quickNES->getLowMemSize());

    JAFFAR_THROW_LOGIC("Property name: '%s' not found in emulator '%s'", propertyName.c_str(), getName().c_str());
  }

  __INLINE__ void enableStateProperty(const std::string& property) { _quickNES->enableStateBlock(property); }

  __INLINE__ void disableStateProperty(const std::string& property) { _quickNES->disableStateBlock(property); }
  ...
  __INLINE__ void enableRendering() override { _quickNES->enableRendering(); }

  __INLINE__ void disableRendering() override { _quickNES->disableRendering(); }

  __INLINE__ void updateRendererState(const size_t stepIdx, const std::string input) override
  {
    saveBlit(_quickNES->getInternalEmulatorPointer(), _curBlit, NES_VIDEO_PALETTE, 0, 0, 0, 0);
  }
