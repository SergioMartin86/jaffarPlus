...
public:
  ...
  {
    for (uint8_t i = 0; i < _rowCount; i++)
      for (uint8_t j = 0; j < _colCount; j++) setPiece({i, j}, 0x00);
    for (const auto& piece : _initialPieces) setPiece({piece.pos.row, piece.pos.col}, piece.type);
  }

  __INLINE__ void setPiece(const piecePos_t pos, const uint8_t type) { _grid[pos.row * _colCount + pos.col] = type; }
  ...
  __INLINE__ piecePos_t getUpPiecePos(const piecePos_t pos) const
  {
    auto nextPos = pos;
    if (nextPos.row == 0)
      nextPos.row = _rowCount - 1;
    else
      nextPos.row = nextPos.row - 1;
    return nextPos;
  }
  ...
  __INLINE__ uint8_t getPiece(const piecePos_t pos) const { return _grid[pos.row * _colCount + pos.col]; }
  ...
  __INLINE__ std::vector<piecePath_t> calculatePipePath() const
  {
    ...
    while (true)
    {
      ...
      if (currentDirection == emulator::PipeBot::direction_t::left && nextPieceType.LRedirection != emulator::PipeBot::direction_t::none)
        nextDirection = nextPieceType.LRedirection;
      if (currentDirection == emulator::PipeBot::direction_t::right && nextPieceType.RRedirection != emulator::PipeBot::direction_t::none)
        nextDirection = nextPieceType.RRedirection;
      ...
      if (currentDirection == emulator::PipeBot::direction_t::down && nextPieceType.DRedirection != emulator::PipeBot::direction_t::none)
        nextDirection = nextPieceType.DRedirection;
  ...
  __INLINE__ void printInfo() const override
  {
    ...
    for (uint8_t i = 0; i < _rowCount; i++)
    {
      ...
      {
        const auto pieceType = getPiece({i, j});
        if (_pieceTypes.contains(pieceType) == false)
        {
          printf("Did not recognize next piece type: 0%02X", pieceType);
          break;
        }
