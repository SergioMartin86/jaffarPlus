...
private:
  ...
  __INLINE__ void stateUpdatePostHook() override { _remainingDistance = (uint16_t)*_spiderPosY1 * 30 + (uint16_t)*_spiderPosY2; }

  __INLINE__ void serializeStateImpl(jaffarCommon::serializer::Base& serializer) const override { serializer.push(&_remainingDistance, sizeof(_remainingDistance)); }

  __INLINE__ void deserializeStateImpl(jaffarCommon::deserializer::Base& deserializer) { deserializer.pop(&_remainingDistance, sizeof(_remainingDistance)); }
  ...
  __INLINE__ void getAdditionalAllowedInputs(std::vector<InputSet::inputIndex_t>& allowedInputSet) override
  {
    if (*_frameType == 255)
    {
      allowedInputSet.insert(allowedInputSet.end(), {_inputNull});
      return;
    }
    if (*_webLength == 0)
    {
      allowedInputSet.insert(allowedInputSet.end(), {_inputU, _inputUB, _inputULB, _inputURB});
      return;
    }
