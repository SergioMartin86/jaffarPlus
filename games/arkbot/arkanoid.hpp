...
class Arkanoid final : public jaffarPlus::Game
{
  ...
  __INLINE__ bool enemyInRange (const Enemy& enemy) const
  {
      return enemy.active && GameConsts::PaddleTop + 4 >= enemy.pos.y
  ...
  __INLINE__ bool ballInRange (const Ball& ball) const
   {
      return (ball.pos.y + 7 >= GameConsts::PaddleTop
  ...
  {
    return 
        _arkState->spawnedPowerup == Powerup::Multiball
        && _arkState->powerupPos.y + 8 >= GameConsts::PaddleTop
        && GameConsts::PaddleTop + 4 >= _arkState->powerupPos.y;
  }
