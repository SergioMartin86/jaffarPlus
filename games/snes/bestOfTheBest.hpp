...
private:
  ...
  __INLINE__ void computeAdditionalHashing(MetroHash128& hashEngine) const override { hashEngine.Update(&_lowMem[0x200], 0x100); }

  // Updating derivative values after updating the internal state
  __INLINE__ void stateUpdatePostHook() override { _reflexesGained = (float)*_reflexes - (float)_initialReflexes + 0.25 * (float)*_reflexHits; }
