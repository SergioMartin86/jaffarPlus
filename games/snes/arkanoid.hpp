...
class Arkanoid final : public jaffarPlus::Game
{
  ...
  {
    hashEngine.Update(&_lowMem[0x0000], 0x1000); 
  }
  ...
  __INLINE__ void stateUpdatePostHook() override
   {
    ...
    _lowestBallPosX = 0;
    _lowestBallPosY = 0;
    if (*_ball1State > 0) if (*_ball1PosY > _lowestBallPosY) { _lowestBallPosX = *_ball1PosX; _lowestBallPosY = *_ball1PosY; _lowestBallVelY = *_ball1VelY; }

    // Only count them if they're not the M powerup trails
    if (*_ballPowerUp != _ARKANOID_BALL_M_POWERUP)
    {
      if (*_ball2State > 0) if (*_ball2PosY > _lowestBallPosY) { _lowestBallPosX = *_ball2PosX; _lowestBallPosY = *_ball2PosY; _lowestBallVelY = *_ball2VelY; }
      if (*_ball3State > 0) if (*_ball3PosY > _lowestBallPosY) { _lowestBallPosX = *_ball3PosX; _lowestBallPosY = *_ball3PosY; _lowestBallVelY = *_ball3VelY; }
      if (*_ball4State > 0) if (*_ball4PosY > _lowestBallPosY) { _lowestBallPosX = *_ball4PosX; _lowestBallPosY = *_ball4PosY; _lowestBallVelY = *_ball4VelY; }
      if (*_ball5State > 0) if (*_ball5PosY > _lowestBallPosY) { _lowestBallPosX = *_ball5PosX; _lowestBallPosY = *_ball5PosY; _lowestBallVelY = *_ball5VelY; }
      if (*_ball6State > 0) if (*_ball6PosY > _lowestBallPosY) { _lowestBallPosX = *_ball6PosX; _lowestBallPosY = *_ball6PosY; _lowestBallVelY = *_ball6VelY; }
      if (*_ball7State > 0) if (*_ball7PosY > _lowestBallPosY) { _lowestBallPosX = *_ball7PosX; _lowestBallPosY = *_ball7PosY; _lowestBallVelY = *_ball7VelY; }
      if (*_ball8State > 0) if (*_ball8PosY > _lowestBallPosY) { _lowestBallPosX = *_ball8PosX; _lowestBallPosY = *_ball8PosY; _lowestBallVelY = *_ball8VelY; }
    }
    ...
    // Enemy-borne balls
    for(size_t i = 0; i < 10; i++) if (_lowMem[0x01DC + i] == 1) _activeBalls++;
    ...
    for (size_t j = 0; j < _ARKANOID_COLUMN_COUNT; j++)
    {
      ...
      switch(_blocks[idx])
      {
        case 0x22: { _remainingHits += 2;  _remainingBlocks++; break; } // Silver Block 
        case 0x23: { _remainingHits += 3;  _remainingBlocks++; break; } // Silver Block 
        case 0x24: { _remainingHits += 4;  _remainingBlocks++; break; } // Silver Block 
        case 0x25: { _remainingHits += 5;  _remainingBlocks++; break; } // Silver Block 
        case 0x26: { _remainingHits += 6;  _remainingBlocks++; break; } // Silver Block 
        case 0x27: { _remainingHits += 7;  _remainingBlocks++; break; } // Silver Block 
        case 0x28: { _remainingHits += 8;  _remainingBlocks++; break; } // Silver Block 
        case 0x2A: { _remainingHits += 9;  _remainingBlocks++; break; } // Silver Block 
        case 0x2B: { _remainingHits += 10; _remainingBlocks++; break; } // Silver Block 
        case 0x2C: { _remainingHits += 11; _remainingBlocks++; break; } // Silver Block 
        case 0x2D: { _remainingHits += 12; _remainingBlocks++; break; } // Silver Block 
        case 0x2E: { _remainingHits += 13; _remainingBlocks++; break; } // Silver Block 
        case 0x2F: { _remainingHits += 14; _remainingBlocks++; break; } // Silver Block 
...
__INLINE__ void getAdditionalAllowedInputs(std::vector<InputSet::inputIndex_t>& allowedInputSet) override
  {
    ...
    for (int i = 0; i < _ARKANOID_COLUMN_COUNT; i++) if (columnTileTarget[i] == target_t::tile) targetsAvailable = true;
    ...
    if (_activeShots < 2) // And still can shoot
    {
      ...
      if (*_paddlePosX == columnPaddlePos[0]) if (columnTileTarget[0] == target_t::tile || columnTileTarget[1] == target_t::tile) addFireInput = true;
      if (*_paddlePosX == columnPaddlePos[1]) if (columnTileTarget[1] == target_t::tile || columnTileTarget[2] == target_t::tile) addFireInput = true;
      if (*_paddlePosX == columnPaddlePos[2]) if (columnTileTarget[2] == target_t::tile || columnTileTarget[3] == target_t::tile) addFireInput = true;
      if (*_paddlePosX == columnPaddlePos[3]) if (columnTileTarget[3] == target_t::tile || columnTileTarget[4] == target_t::tile) addFireInput = true;
      if (*_paddlePosX == columnPaddlePos[4]) if (columnTileTarget[4] == target_t::tile || columnTileTarget[5] == target_t::tile) addFireInput = true;
      if (*_paddlePosX == columnPaddlePos[5]) if (columnTileTarget[5] == target_t::tile || columnTileTarget[6] == target_t::tile) addFireInput = true;
      if (*_paddlePosX == columnPaddlePos[6]) if (columnTileTarget[6] == target_t::tile || columnTileTarget[7] == target_t::tile) addFireInput = true;
      if (*_paddlePosX == columnPaddlePos[7]) if (columnTileTarget[7] == target_t::tile || columnTileTarget[8] == target_t::tile) addFireInput = true;
      if (*_paddlePosX == columnPaddlePos[8]) if (columnTileTarget[8] == target_t::tile || columnTileTarget[9] == target_t::tile) addFireInput = true;

      // Cancelling if there is a blocking golden block on the left
      if (*_paddlePosX == columnPaddlePos[0]) if (columnTileTarget[0] == target_t::blocked && columnTileRow[0] > columnTileRow[1]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[1]) if (columnTileTarget[1] == target_t::blocked && columnTileRow[1] > columnTileRow[2]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[2]) if (columnTileTarget[2] == target_t::blocked && columnTileRow[2] > columnTileRow[3]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[3]) if (columnTileTarget[3] == target_t::blocked && columnTileRow[3] > columnTileRow[4]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[4]) if (columnTileTarget[4] == target_t::blocked && columnTileRow[4] > columnTileRow[5]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[5]) if (columnTileTarget[5] == target_t::blocked && columnTileRow[5] > columnTileRow[6]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[6]) if (columnTileTarget[6] == target_t::blocked && columnTileRow[6] > columnTileRow[7]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[7]) if (columnTileTarget[7] == target_t::blocked && columnTileRow[7] > columnTileRow[8]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[8]) if (columnTileTarget[8] == target_t::blocked && columnTileRow[8] > columnTileRow[9]) addFireInput = false;

      // Cancelling if there is a blocking golden block on the right
      if (*_paddlePosX == columnPaddlePos[0]) if (columnTileTarget[1] == target_t::blocked && columnTileRow[1] > columnTileRow[0]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[1]) if (columnTileTarget[2] == target_t::blocked && columnTileRow[2] > columnTileRow[1]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[2]) if (columnTileTarget[3] == target_t::blocked && columnTileRow[3] > columnTileRow[2]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[3]) if (columnTileTarget[4] == target_t::blocked && columnTileRow[4] > columnTileRow[3]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[4]) if (columnTileTarget[5] == target_t::blocked && columnTileRow[5] > columnTileRow[4]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[5]) if (columnTileTarget[6] == target_t::blocked && columnTileRow[6] > columnTileRow[5]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[6]) if (columnTileTarget[7] == target_t::blocked && columnTileRow[7] > columnTileRow[6]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[7]) if (columnTileTarget[8] == target_t::blocked && columnTileRow[8] > columnTileRow[7]) addFireInput = false;
      if (*_paddlePosX == columnPaddlePos[8]) if (columnTileTarget[9] == target_t::blocked && columnTileRow[9] > columnTileRow[8]) addFireInput = false;
    }
    ...
    if (*_powerUp1Type == _ARKANOID_FALLING_EIGHTBALL_POWERUP
     || *_powerUp1Type == _ARKANOID_FALLING_LASER_POWERUP
    ...
    if (*_powerUp2Type == _ARKANOID_FALLING_EIGHTBALL_POWERUP
     || *_powerUp2Type == _ARKANOID_FALLING_LASER_POWERUP
