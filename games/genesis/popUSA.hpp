...
class PrinceOfPersiaUSA final : public jaffarPlus::Game
{
  ...
  __INLINE__ void computeAdditionalHashing(MetroHash128 &hashEngine) const override 
  {
    ...
    // for (size_t i = 0x0000; i < 0xFFA0; i++) if (i != 0x0FDE && i != 0x0FDF)  hashEngine.Update(_workRAM[i]);
     { uint64_t start = 0x0D70; uint64_t end = 0x0F40; hashEngine.Update(&_workRAM[start], end - start); }
     { uint64_t start = 0x0F50; uint64_t end = 0x1000; hashEngine.Update(&_workRAM[start], end - start); }
     { uint64_t start = 0x0020; uint64_t end = 0x0100; hashEngine.Update(&_workRAM[start], end - start); }
