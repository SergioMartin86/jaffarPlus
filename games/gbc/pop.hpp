...
class PrinceOfPersia final : public jaffarPlus::Game
{
  ...
  __INLINE__ float calculateGameSpecificReward() const
  {
    ...
    if (_kidDirectionMagnet.room == _actualKidRoom && _kidPointXMagnet.room == *_kidRoom && *_kidDirection == _kidDirectionMagnet.direction) reward += _kidDirectionMagnet.intensity;

    if (*_guardHP > 0) if (_guardPointXMagnet.room == _actualKidRoom && _kidPointXMagnet.room == *_kidRoom) reward += -1.0 * _guardPointXMagnet.intensity * (_guardDistanceToPointX - 255.0);
  ...
  {
    return jaffarCommon::hash::hash_t({0, *_kidAction});
  }
