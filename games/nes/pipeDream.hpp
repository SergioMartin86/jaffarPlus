...
private:
    ...
    if (_endPieceFound == true)
    {
      ...
      if (_endsHaveMet == false)
      {
        ...
        _distanceBetweenEnds =
            std::abs((int8_t)lastForwardPiecePos->first - (int8_t)lastInversePiecePos->first) + std::abs((int8_t)lastForwardPiecePos->second - (int8_t)lastInversePiecePos->second);
      }
    }
  ...
  __INLINE__ std::vector<piecePos_t> calculatePipePath() const
  {
    ...
    while (true)
    {
      ...
      if (_pieceTypes.contains(nextPieceType) == false)
      {
        printf("Did not recognize next piece type: 0%02X", nextPieceType);
        break;
      }
  ...
  __INLINE__ void playerPrintCommands() const override { jaffarCommon::logger::log("[J+] t: Print Initial Info\n"); };
  ...
  __INLINE__ bool playerParseCommand(const int command)
  {
    if (command == 't')
    {
      ...
      for (auto itr = _initialPieces.begin(); itr != _initialPieces.end(); itr++)
        jaffarCommon::logger::log(" { \"Row\": %3u, \"Col\": %3u, \"Type\": %3u }%s ", itr->first, itr->second, _gridStart[itr->first * __PIPE_DREAM_GRID_COLS + itr->second],
                                  (itr->first == _initialPieces.rbegin()->first) && (itr->second == _initialPieces.rbegin()->second) ? "" : ",");
      jaffarCommon::logger::log(" ], ");
