...
private:
  ...
  __INLINE__ void ruleUpdatePostHook() override
  {
    ...
    if (_useTrace == true)
    {
      ...
      _traceDistancePlayer = sqrtf(_traceDistancePlayerPosX * _traceDistancePlayerPosX + _traceDistancePlayerPosY * _traceDistancePlayerPosY);

      _traceDistanceStar = sqrtf(_traceDistanceStarPosX * _traceDistanceStarPosX + _traceDistanceStarPosY * _traceDistanceStarPosY);
    }
  }
  ...
  __INLINE__ void serializeStateImpl(jaffarCommon::serializer::Base& serializer) const override
  {
    ...
    serializer.pushContiguous(&_starUse, sizeof(_starUse));
  }
  ...
  __INLINE__ void deserializeStateImpl(jaffarCommon::deserializer::Base& deserializer)
  {
    ...
    deserializer.popContiguous(&_starUse, sizeof(_starUse));
  }
  ...
  void printInfoImpl() const override
  {
    ...
    if (std::abs(_lastInputStepMagnet) > 0.0f) { jaffarCommon::logger::log("[J+]  + Last Input Step Magnet                   Intensity: %.5f\n", _lastInputStepMagnet); }

    if (_useTrace == true)
    {
      if (std::abs(_traceMagnet.playerIntensity) > 0.0f)
      {
        jaffarCommon::logger::log("[J+]  + Player Trace Magnet                      Intensity: %.5f, Step: %u, X: %3.3f, Y: %3.3f\n", _traceMagnet.playerIntensity, _traceStep,
                                  _traceTargetPlayerPosX, _traceTargetPlayerPosY);
      ...
      {
        jaffarCommon::logger::log("[J+]  + Star Trace Magnet                       Intensity: %.5f, Step: %u, X: %3.3f, Y: %3.3f\n", _traceMagnet.starIntensity, _traceStep,
                                  _traceTargetStarPosX, _traceTargetStarPosY);
  ...
  __INLINE__ bool playerParseCommand(const int command)
  {
    ...
    if (_isDumpingTrace == true)
      _traceDumpString += std::to_string(_playerPosX) + std::string(" ") + std::to_string(_playerPosY) + std::string(" ") + std::to_string(_starPosX) + std::string(" ") +
                          std::to_string(_starPosY) + std::string("\n");
