...
private:
  __INLINE__ void registerGameProperties() override
  {
    ...
    // Thanks to https://datacrystal.romhacking.net/wiki/Super_Player_Bros.:RAM_map and https://tasvideos.org/GameResources/NES/SuperMarioBros for helping me find some of these
    // items
    ...
    // Swimming flag (0x0704): nonzero only in water (verified land=0 / swim=1). Used by allowed-input
    // conditions to prune water-ineffective inputs (B/run and Down are no-ops in water) and to still
    // offer A (the swim stroke) while sinking.
    registerGameProperty("Swimming Flag", &_lowMem[0x0704], Property::datatype_t::dt_uint8, Property::endianness_t::little);
...
#define __HASH_METHOD_1
    ...
    // [HASH-RESTRICT safe-only] dropped (cosmetic 0x0001 animation): hashEngine.Update(*_playerAnimation);
    ...
    // [HASH-RESTRICT safe-only] dropped (cosmetic 0x0702 walk-anim): hashEngine.Update(*_playerWalkingMode);
    // [HASH-RESTRICT safe-only] dropped (cosmetic 0x070C walk-anim): hashEngine.Update(*_playerWalkingDelay);
    // [HASH-RESTRICT safe-only] dropped (cosmetic 0x070D walk-anim): hashEngine.Update(*_playerWalkingFrame);
    ...
    hashEngine.Update(
        _lowMem[0x071E]); // 	Column Sets . Counts back, done when this is FF. you can see the level being built in memory up as this counter progresses, one frame at a time.
    hashEngine.Update(_lowMem[0x071F]); // 	AreaParser TaskNumber, runs from 4 to 0.
    hashEngine.Update(_lowMem[0x0722]); // 	Player HitDetect Flag. Determines whether allow Player to Pass through from Bricks or not. (Settting it to 0xFF will cause Player to
                                        // Penetrate through Walls/Bricks)
    ...
    hashEngine.Update(_lowMem[0x0755]); // 	Player_Position For Scroll. It moves up to 0x70 (sometimes even 0x72) when player moves. When this register is frozen, NOTHING changes
                                        // in the level, not even internally.
  ...
  void printInfoImpl() const override
  {
    ...
    jaffarCommon::logger::log("[J+]  + Timers:                  %02u %02u %02u %02u %02u %02u %02u %02u %02u %02u %02u %02u %02u %02u %02u %02u\n", *_animationTimer,
                              *_jumpSwimTimer, *_runningTimer, *_blockBounceTimer, *_sideCollisionTimer, *_jumpspringTimer, *_gameControlTimer, *_climbSideTimer, *_enemyFrameTimer,
                              *_frenzyEnemyTimer, *_bowserFireTimer, *_stompTimer, *_airBubbleTimer, *_multiCoinBlockTimer, *_invincibleTimer, *_starTimer);
    ...
    if (_useTrace == true)
    {
      ...
      if (std::abs(_traceMagnet.intensityPosX) > 0.0f)
        jaffarCommon::logger::log("[J+]    + Pos X    / Intensity  %8.3f / Target %8.3f / Distance %7.3f / Effect %7.4f\n", _traceMagnet.intensityPosX, _tracePosTargetX,
                                  _tracePosDistanceX, _tracePosEffectX);
      if (std::abs(_traceMagnet.intensityPosY) > 0.0f)
        jaffarCommon::logger::log("[J+]    + Pos Y    / Intensity  %8.3f / Target %8.3f / Distance %7.3f / Effect %7.4f\n", _traceMagnet.intensityPosY, _tracePosTargetY,
                                  _tracePosDistanceY, _tracePosEffectY);
      if (std::abs(_traceMagnet.intensityVelX) > 0.0f)
        jaffarCommon::logger::log("[J+]    + Vel X    / Intensity  %8.3f / Target %8.3f / Distance %7.3f / Effect %7.4f\n", _traceMagnet.intensityVelX, _traceVelTargetX,
                                  _traceVelDistanceX, _traceVelEffectX);
      if (std::abs(_traceMagnet.intensityVelY) > 0.0f)
        jaffarCommon::logger::log("[J+]    + Vel Y    / Intensity  %8.3f / Target %8.3f / Distance %7.3f / Effect %7.4f\n", _traceMagnet.intensityVelY, _traceVelTargetY,
                                  _traceVelDistanceY, _traceVelEffectY);
      if (std::abs(_traceMagnet.intensityScreenX) > 0.0f)
        jaffarCommon::logger::log("[J+]    + Screen X / Intensity  %8.3f / Target %8.3f / Distance %7.3f / Effect %7.4f\n", _traceMagnet.intensityScreenX, _traceScreenPosTargetX,
                                  _traceScreenPosDistanceX, _traceScreenPosEffectX);
    }
  }
  ...
  bool parseRuleActionImpl(Rule& rule, const std::string& actionType, const nlohmann::json& actionJs) override
  {
    ...
    if (actionType == "Set Trace Magnet")
    {
      ...
      rule.addAction(
          [=, this]()
          {
            this->_traceMagnet = traceMagnet_t{.intensityPosX    = intensityPosX,
  ...
  __INLINE__ jaffarCommon::hash::hash_t getStateInputHash() override
  {
    ...
    inputHash.second += _lowMem[0x0000] + _lowMem[0x0002] + _lowMem[0x0003] + _lowMem[0x0005] + _lowMem[0x03AD] + _lowMem[0x03B8] + _lowMem[0x0400] + _lowMem[0x0416] +
                        _lowMem[0x0433] + _lowMem[0x0450] + _lowMem[0x0456] + _lowMem[0x0490] + _lowMem[0x04AC] + _lowMem[0x06D5] + _lowMem[0x0700] + _lowMem[0x0701] +
                        _lowMem[0x0702] + _lowMem[0x0704] + _lowMem[0x0705] + _lowMem[0x0709] + _lowMem[0x070A] + _lowMem[0x070B] + _lowMem[0x070C] + _lowMem[0x070D] +
                        _lowMem[0x0714] + _lowMem[0x071C] + _lowMem[0x071D] + _lowMem[0x071E] + _lowMem[0x071F] + _lowMem[0x0722] + _lowMem[0x0723] + _lowMem[0x0754] +
                        _lowMem[0x0755];
  ...
  __INLINE__ bool playerParseCommand(const int command)
  {
    ...
    if (_isDumpingTrace == true)
      _traceDumpString += std::to_string(_playerPosX) + std::string(" ") + std::to_string(_playerPosY) + std::string(" ") + std::to_string(*_playerVelX1) + std::string(" ") +
                          std::to_string(*_playerVelY1) + std::string(" ") + std::to_string(_playerScreenOffset) + std::string("\n");
