...
class SaintSeiyaOugonDensetsu final : public jaffarPlus::Game
{
  ...
  {
      _fightsCompleted = 0;
      for (size_t i = 0; i < _SAINT_SEIYA_FIGHT_COUNT; i++) if (_fightVector[i] > 127) _fightsCompleted++;
  }
  ...
  __INLINE__ float calculateGameSpecificReward() const
  {
    ...
    for (size_t i = 0; i < _SAINT_SEIYA_FIGHT_COUNT; i++) if (_fightVector[i] < 200) if (_fightVector[i] > maxFightValue) maxFightValue = _fightVector[i];
    reward += maxFightValue * 0.001 * _fightsCompletedMagnet;
    for (size_t i = 0; i < _SAINT_SEIYA_FIGHT_COUNT; i++) if (_fightVector[i] > 0) reward += _fightsCompletedMagnet * .00001;
  ...
  void printInfoImpl() const override
  {
    ...
    if (std::abs(_fightsCompletedMagnet) > 0.0f)
    {
      jaffarCommon::logger::log("[J+]  + Fights Completed Magnet                   Intensity: %.5f\n", _fightsCompletedMagnet);
    }
  }
