...
extern "C" __thread struct miniPopState_t gameState;
...
typedef int  parse_grmode_t(void);
typedef void  init_timer_t(int frequency);
typedef void  parse_cmdline_sound_t(void);
typedef void  set_hc_pal_t(void);
typedef void  show_loading_t(void);
typedef int  set_joy_mode_t(void);
typedef void  init_copyprot_dialog_t(void);
typedef void init_record_replay_t(void);
typedef void init_menu_t(void);
typedef word  prandom_t(word max);
...
typedef void  set_pal_256_t(rgb_type far *source);
typedef void  set_pal_t(int index, int red, int green, int blue, int vsync);
typedef chtab_type *__pascal load_sprites_from_file_t(int resource, int palette_bits, int quit_on_error);
typedef void  close_dat_t(dat_type far *pointer);
typedef void init_lighting_t(void);
typedef void load_all_sounds_t(void);
typedef void  hof_read_t(void);
typedef void  release_title_images_t(void);
typedef void  free_optsnd_chtab_t(void);
typedef void  load_kid_sprite_t(void);
typedef void  load_lev_spr_t(int level);
typedef void  load_level_t(void);
typedef void  pos_guards_t(void);
typedef void  clear_coll_rooms_t(void);
typedef void  clear_saved_ctrl_t(void);
typedef void  do_startpos_t(void);
typedef void  find_start_level_door_t(void);
typedef int  check_sound_playing_t(void);
typedef int  do_paused_t(void);
typedef void idle_t(void);
typedef void  stop_sounds_t(void);
typedef void  show_copyprot_t(int where);
typedef void reset_timer_t(int timer_index);
typedef void  free_peels_t(void);
typedef int  play_level_2_t(void);
typedef void  timers_t(void);
typedef void  start_game_t(void);
typedef void  play_frame_t(void);
typedef void  draw_game_frame_t(void);
...
typedef void  load_room_links_t(void);
typedef void set_timer_length_t(int timer_index, int length);
typedef void  draw_level_first_t(void);
typedef void  play_level_t(int level_number);
...
typedef void  process_trobs_t(void);
typedef void  do_mobs_t(void);
typedef void  check_skel_t(void);
typedef void  check_can_guard_see_kid_t(void);
typedef void  check_mirror_t(void);
typedef void  init_copyprot_t(void);
typedef void  alter_mods_allrm_t(void);
typedef void  start_replay_t(void);
typedef void  display_text_bottom_t(const char near *text);
typedef void  redraw_screen_t(int drawing_different_room);
typedef void  anim_tile_modif_t();
...
extern "C" anim_tile_modif_t anim_tile_modif;
