...
void quicknesInstance::initialize(const char* romFile, const char* saveFile, const bool headlessMode, const bool fastVdp)
{
// int argc = 4;
// char* argv[4];
//
...
void quicknesInstance::reset()
{
// restartGenesis();
// loadState(_startStateData);
}

void quicknesInstance::setRNGValue(const uint32_t& rngValue)
{
...
void quicknesInstance::setHPValue(const uint8_t& hp)
{
...
void quicknesInstance::setKidXValue(const uint16_t& x)
{
...
gameStateStruct quicknesInstance::getGameState(const uint8_t* state)
{
...
uint64_t quicknesInstance::computeHash()
{
  MetroHash64 hash;
...
void quicknesInstance::printState()
{
...
std::vector<uint8_t> quicknesInstance::getPossibleMoveIds(const gameStateStruct& gameState)
{
...
int quicknesInstance::playFrame(const std::string& move)
{
...
void quicknesInstance::loadState(const uint8_t* state)
{
...
void quicknesInstance::saveState(uint8_t* state)
{
// memcpy(state, _stateData, _stateSize);
}

void quicknesInstance::redraw()
{
// redraw();
}

void quicknesInstance::finalize()
{
// finalize();
}
